 ...
 * @param {Object} [params]
 * @param {Boolean} [params.splitQueryStringFromCurrentUrl=false]
 * the parameter determines whether the current URL
...
WebDriver.prototype.waitForUrlChange =
	function(oldUrl, newUrl, params, callback) {
		...
		params = _({
			splitQueryStringFromCurrentUrl: false
		}).extend(params);
		...
		self.waitFor(
			function(waitCallback) {
				self.getUrl(function(err, url) {
					...
					if (params.splitQueryStringFromCurrentUrl) {
						// remove query string
						url = url.replace(/\?.*$/, '');
					}
