...
function WebDriver(params) {
	...
	self.sessionBasePath = '/wd/hub/session';
	self.desiredCapabilities = utils.extend({
		browserName: 'firefox',
		version: '',
		javascriptEnabled: true,
		platform: 'ANY'
	}, params.desiredCapabilities);
	self.defaults = utils.extend({
		using: 'css selector'
	}, params.defaults);
	self.logMethodCalls = params.logMethodCalls;
	self.timeouts = utils.extend({
		// protocol timeouts
		'page load': 3500,
		script: 1000,
		implicit: 0,
	...
	self._protocolTimeoutsHash = {'page load': 1, script: 1, implicit: 1};
...
WebDriver.prototype._getIds = function(selector, params, callback) {
	...
	function parametrizeError(err) {
		...
		err.message = err.message
			.replace('${element}', selector + ' at ')
			.replace('${using}', params.using);
		return err;
	}
};
