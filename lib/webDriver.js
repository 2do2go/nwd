...
var jqueryGetElementsInjection = utils.getInjectionSource(function() {
	...
	function ___nwdJqGetElements(selector, parent, chain, cssFilter) {
		cssFilter = cssFilter || {};
		var elements = null;
		...
		self.execute(
			(addInjection || '') + utils.getInjectionSource(function() {
				if (typeof window.___nwdJquery !== 'function') {
					...
					// disable requirejs define to prevent jquery module defining
					var oldDefine = window.define;
					delete window.define;
					// inject jquery
					___nwdJquery();
					window.___nwdJquery = jQuery.noConflict(true);
					// return old requirejs define
				...
				return ___nwdJqGetElements.apply(null, arguments);
			}),
			[
				selector,
				params.parentId && {ELEMENT: params.parentId},
				params.chain,
				params.cssFilter
			],
			false,
