...
WebDriver.prototype._log = function(message, content) {
	...
	if (!content) {
		...
		console.log(
			utils.colors.dkgray + '[' + dateString + ']: ' + utils.colors.reset, message,
			'\t', trimStringLength(JSON.stringify(content), 256, true));
	}
};
...
WebDriver.prototype._cmd = function(params, callback) {
	...
	this._log(
		utils.colors.brown + 'DATA\t\t ' + utils.colors.reset +
		trimStringLength(stringData, 256, true)
	);
	var req = request(requestParams, function(err, res) {
		...
		self._log(utils.colors.teal + 'RESULT\t' + utils.colors.reset, res.data);
