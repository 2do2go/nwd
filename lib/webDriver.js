...
WebDriver.prototype.setTimeout = function(type, timeout, callback) {
	...
	} else if (type in this.timeouts) {
		self.timeouts[type] = timeout;
		callback(null, self);
	} else {
		callback(new Error('Unrecognized timeout type: ' + type));
	}
};
...
WebDriver.prototype.waitForElement = function(selector, params, callback) {
	...
	self.waitFor(
		...
		params.noError ? null : 'waiting for element ' + selector,
		callback
	);
};
...
WebDriver.prototype.waitFor = function(func, errorMessage, callback) {
	var self = this,
		start = Date.now(),
		timeout = self.timeouts.waitFor,
		delay = timeout / 30,
	...
	function execute() {
		...
		setTimeout(function() {
			if (!isDone) {
				...
				callback(errorMessage ? new Error(
					'Timeout (' + timeout + ' ms) exceeded while ' + errorMessage
				) : null);
			}
		}, timeout);
	};
