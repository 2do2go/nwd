...
/**
 * Element constructor
 * @param {String} id Element id (e.g. returned by WebDriver.get)
 ...
 * @param {Object} offset Object with x, y keys. If offset is not set the mouse
...
/**
 * `moveTo` to the current element and call `WebDriver.mouseDown` (accept same
 * arguments, see driver method desctiption for details)
 */
...
/**
 * `moveTo` to the current element and call `WebDriver.mouseUp` (accept same
 * arguments, see driver method desctiption for details)
 */
 ...
 * Accept same parameters as `WebDriver.get`.
 */
 ...
 * Accept same parameters as `WebDriver.getList`.
 */
...
WebElement.prototype.saveState = function(callback) {
	var self = this;
	self.getAttr('outerHTML', function(err, html) {
		if (err) return callback(err);
		self.state = html;
		callback(null, self);
	});
...
WebElement.prototype.waitForStateChange = function(callback) {
	var self = this;
	if ('state' in self === false) {
		return callback(new errors.NoCurrentStateError());
	}
	self.driver.waitFor(
		function(waitCallback) {
			self.getAttr('outerHTML', function(err, html) {
				if (err) return callback(err);
				waitCallback(self.state !== html);
			});
		}, {
			errorMessage: 'waiting for element ' + self.id + ' state change',
			timeout: self.driver.timeouts.waitForStateChange
		},
		function(err) {
			callback(err, self);
		}
